...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 20 21:50:02 2023
...
df_Utility_Disconnects = df_Utility_Disconnects[df_Utility_Disconnects['Year'] <= 2013]
...
df_rental_new = df_rental_new[df_rental_new['Permit Issue Year'] <= 2013]
...
# convert year to age
df_rental_new['Permit Issue Year'] = 2013-df_rental_new['Permit Issue Year']
...
df_SFIncidents_new = df_SFIncidents_new.loc[(df_SFIncidents_new['Year'] >=2013) &  (df_SFIncidents_new['Year'] <=2017)]

df_fire = df_SFIncidents_new[df_SFIncidents_new['Code']==111]

df_fire = df_fire[['Location']]

# # filter by year for last incident before or on 2011

fire = df_SFIncidents_new.loc[(df_SFIncidents_new['Year'] <=2013)]
fire = fire.groupby(['Location'], sort=False)['Year'].max().reset_index()
fire['Year'] = 2013 - fire['Year']
...
ready_anytime['ready_anytime'] = 0 # as 2012 or before doesn't have any inspection data, for those years 'ready_anytime'=0 for all addresses 
...
# remove data after 2013
df_foreclosure_new = df_foreclosure_new[df_foreclosure_new['YEAR'] <= 2013]
