...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 20 22:52:20 2023
...
df_offenses_new = df_offenses_new[df_offenses_new['Year'] <= 2019]
...
df_Utility_Disconnects = df_Utility_Disconnects[df_Utility_Disconnects['Year'] <= 2019]
...
df_rental_new = df_rental_new[df_rental_new['Permit Issue Year'] <= 2019]
...
# convert year to age
df_rental_new['Permit Issue Year'] = 2019-df_rental_new['Permit Issue Year']
...
df_SFIncidents_new = df_SFIncidents_new.loc[(df_SFIncidents_new['Year'] >=2019) &  (df_SFIncidents_new['Year'] <=2023)]

df_fire = df_SFIncidents_new[df_SFIncidents_new['Code']==111]

df_fire = df_fire[['Location']]

# # filter by year for last incident before or on 2019

fire = df_SFIncidents_new.loc[(df_SFIncidents_new['Year'] <=2019)]
fire = fire.groupby(['Location'], sort=False)['Year'].max().reset_index()
fire['Year'] = 2019 - fire['Year']
...
# remove data after 2019
ready_anytime = ready_anytime[ready_anytime['Last Inspected']<=2019]
...
# remove data after 2019
df_foreclosure_new = df_foreclosure_new[df_foreclosure_new['YEAR'] <= 2019]
