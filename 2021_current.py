...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 20 22:59:33 2023
...
df_offenses_new = df_offenses_new[df_offenses_new['Year'] <= 2021]
...
df_Utility_Disconnects = df_Utility_Disconnects[df_Utility_Disconnects['Year'] <= 2021]
...
df_rental_new = df_rental_new[df_rental_new['Permit Issue Year'] <= 2021]
...
# convert year to age
df_rental_new['Permit Issue Year'] = 2021-df_rental_new['Permit Issue Year']
...
df_SFIncidents_new = df_SFIncidents_new.loc[(df_SFIncidents_new['Year'] >=2021) &  (df_SFIncidents_new['Year'] <=2025)]

df_fire = df_SFIncidents_new[df_SFIncidents_new['Code']==111]

df_fire = df_fire[['Location']]

# # filter by year for last incident before or on 2021

fire = df_SFIncidents_new.loc[(df_SFIncidents_new['Year'] <=2021)]
fire = fire.groupby(['Location'], sort=False)['Year'].max().reset_index()
fire['Year'] = 2021 - fire['Year']
...
# remove data after 2021
ready_anytime = ready_anytime[ready_anytime['Last Inspected']<=2021]
...
# remove data after 2021
df_foreclosure_new = df_foreclosure_new[df_foreclosure_new['YEAR'] <= 2021]
