...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 20 22:48:09 2023
...
df_offenses_new = df_offenses[['Parcel_Address', 'Offense', 'Date']]
...
df_offenses_new = df_offenses_new[df_offenses_new['Year'] <= 2018]
...
df_Utility_Disconnects = df_Utility_Disconnects[df_Utility_Disconnects['Year'] <= 2018]
...
df_rental_new = df_rental_new[df_rental_new['Permit Issue Year'] <= 2018]
...
# convert year to age
df_rental_new['Permit Issue Year'] = 2018-df_rental_new['Permit Issue Year']
...
df_SFIncidents_new = df_SFIncidents_new.loc[(df_SFIncidents_new['Year'] >=2018) &  (df_SFIncidents_new['Year'] <=2022)]

df_fire = df_SFIncidents_new[df_SFIncidents_new['Code']==111]

df_fire = df_fire[['Location']]

# # filter by year for last incident before or on 2017

fire = df_SFIncidents_new.loc[(df_SFIncidents_new['Year'] <=2018)]
fire = fire.groupby(['Location'], sort=False)['Year'].max().reset_index()
fire['Year'] = 2018 - fire['Year']
...
# remove data after 2018
ready_anytime = ready_anytime[ready_anytime['Last Inspected']<=2018]
...
# remove data after 2018
df_foreclosure_new = df_foreclosure_new[df_foreclosure_new['YEAR'] <= 2018]
